defmodule PingSiteWeb.APIAuthPlugTest do
  use PingSiteWeb.ConnCase
  doctest PingSiteWeb.APIAuthPlug

  alias PingSiteWeb.{APIAuthPlug, Endpoint}
  alias PingSite.{Repo, Users.User}
  alias Plug.Conn

  @pow_config [otp_app: :pingsite]

  setup %{conn: conn} do
    conn = %{conn | secret_key_base: Endpoint.config(:secret_key_base)}

    user =
      Repo.insert!(%User{id: Ecto.UUID.generate(), email: "test@example.com"})
